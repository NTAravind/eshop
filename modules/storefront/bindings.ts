...
function isProductLike(value: unknown): value is Record<string, unknown> {
    ...
    return ('id' in value) && ('name' in value || 'variants' in value || 'images' in value);
}
...
export function resolveBinding(path: string, context: RuntimeContext | BindingContext): unknown {
    ...
    // Allow product.* bindings inside repeaters to prefer scoped items
    if (rootKey === 'product' && '__scope' in context && context.__scope && isProductLike(context.__scope.item)) {
        const scopedProduct = context.__scope.item;
        if (restSegments.length === 0) {
            return scopedProduct;
        }
        return walkObject(scopedProduct, restSegments);
    }
...
// ==================== V2 DUAL-PATH RESOLUTION ====================

import type { StorefrontNode, BindingExpr } from '@/types/storefront-builder';
import { resolveBindingExpr, resolveBindingMap } from './binding-ast';

/**
 * Resolve bindings for a StorefrontNode, supporting both V1 and V2 formats.
 *
 * Priority:
 * 1. If node.bindingMap exists (V2), use AST resolution
 * 2. Otherwise, fall back to node.bindings (V1 string paths)
 *
 * Returns the merged props with resolved binding values.
 */
export function resolveNodeBindings(
    ...
    // V2: Use bindingMap (AST-based)
    if (node.bindingMap && Object.keys(node.bindingMap).length > 0) {
        const resolved = resolveBindingMap(
            node.bindingMap,
            '__scope' in context ? context : { ...context } as BindingContext
        );
...
// Re-export V2 AST utilities for convenience
export { resolveBindingExpr, resolveBindingMap, migrateStringBinding, migrateNodeBindings, validateBindingExpr } from './binding-ast';
