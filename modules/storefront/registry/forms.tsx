...
function DeliveryModeSelector({
    ...
    const { context, setUIState } = runtimeContext || {};
    const uiState = context?.uiState || {};

    // Use selected prop if provided, otherwise fall back to UI state, then first available mode
    // Ensure the selected mode is actually available in the modes array
    let currentMode = selected || uiState.deliveryMode;
    if (!currentMode || !modes.includes(currentMode)) {
        currentMode = modes[0];
    }

    // Sync back to UI state if we fell back to a default and no valid state exists
    React.useEffect(() => {
        if (currentMode && uiState.deliveryMode !== currentMode && !onChange) {
            setUIState?.('deliveryMode', currentMode);
        }
    }, [currentMode, uiState.deliveryMode, onChange, setUIState]);

    const handleClick = (mode: 'DELIVERY' | 'PICKUP') => {
        if (onChange) {
            onChange(mode);
        } else {
            setUIState('deliveryMode', mode);
        }
    };
            ...
            {modes.map((mode) => (
                <button
                    ...
                    style={{
                        flex: 1,
                        padding: '1rem',
                        border: currentMode === mode ? '2px solid var(--primary)' : '1px solid var(--border)',
                        borderRadius: 'var(--radius)',
                        backgroundColor: currentMode === mode ? 'var(--primary)' : 'transparent',
                        color: currentMode === mode ? 'var(--primary-foreground)' : 'var(--foreground)',
                        cursor: 'pointer',
                        fontWeight: 500,
                    }}
                >
                    {mode === 'DELIVERY' ? deliveryLabel : pickupLabel}
                </button>
            ))}
        </div>
    );
            ...
            {orders.map((order) => (
                ...
                >
                    ...
                    <div style={{ textAlign: 'right' }}>
                        <p style={{ fontWeight: 600 }}>{formatCurrency(order.total, currency)}</p>
                        <span
                            style={{
                                fontSize: '0.75rem',
...
export function registerFormComponents() {
    ...
    registerComponent('PlaceOrderButton', PlaceOrderButton as React.ComponentType<BaseComponentProps & Record<string, unknown>>, {
        ...
        propsSchema: {},
        controls: {
            text: { type: 'text', label: 'Button Text', defaultValue: '🛒 Place Order' },
        },
        constraints: { canHaveChildren: false },
        defaults: {
