...
import { defaultTheme } from '@/modules/storefront/defaults/theme';
import {
    findNodeById,
    getNodePath,
    getNodeAtPath,
...
interface EditorState {
    ...
    activeLeftTab: 'pages' | 'components' | 'layers' | 'data';
    activeRightTab: 'properties' | 'style' | 'bindings' | 'actions' | 'children';
    activeBottomTab: 'problems' | 'console' | 'publish';
...
interface EditorActions {
    ...
    // Selection
    select: (nodeId: string | null, childNodeId?: string | null) => void;
...
// Helper functions are imported from '@/shared/utils/tree'
...
export const useEditorStore = create<EditorStore>()(
    immer((set, get) => ({
        ...
        insertNode: (parentId, node, index) => {
            ...
            // Constraint check: can this child type go in this parent?
            const check = canInsert(parent.type, node.type, parent.children.length);
            if (!check.allowed) {
                console.warn(`[Builder] Insert blocked: ${check.reason}`);
                return;
            }

            // Clone node and regenerate ALL IDs (root + descendants)
        ...
        removeNode: (nodeId) => {
            ...
            const newTree = cloneNode(tree);
            const parent = getParentNode(newTree, nodeId);
        ...
        moveNode: (nodeId, targetParentId, index) => {
            ...
            const movedSubtree = findNodeById(tree, nodeId);
            if (movedSubtree && findNodeById(movedSubtree, targetParentId)) return;

            const newTree = cloneNode(tree);

            // Find and remove from current parent
            const currentParent = getParentNode(newTree, nodeId);
            if (!currentParent || !currentParent.children) return;

            const currentIndex = currentParent.children.findIndex((c) => c.id === nodeId);
            if (currentIndex === -1) return;

            // Find target parent for constraint check
            const targetParent = findNodeById(newTree, targetParentId);
            if (!targetParent) return;

            // Constraint check: can this node type go in the target parent?
            const childCount = (targetParent.children?.length ?? 0) -
                (currentParent.id === targetParentId ? 1 : 0); // adjust if same parent
            const check = canInsert(targetParent.type, currentParent.children[currentIndex].type, childCount);
            if (!check.allowed) {
                console.warn(`[Builder] Move blocked: ${check.reason}`);
                return;
            }
        ...
        select: (nodeId, childNodeId) => {
            set((state) => {
                if (nodeId === null) {
                    state.selection = { nodeId: null, childNodeId: null, path: [] };
                } else if (state.tree) {
                    const path = getNodePath(state.tree, nodeId);
                    state.selection = { nodeId, childNodeId: childNodeId || null, path: path || [] };
                }
            });
        },
        ...
        setActiveLeftTab: (tab) => {
            set((state) => {
                ...
                if (tab) state.leftPanelCollapsed = false;
            });
        },
        ...
        setActiveRightTab: (tab) => {
            set((state) => {
                ...
                if (tab) state.rightPanelCollapsed = false;
            });
        },
...
/**
 * Creates a selector for a specific node by ID
 * Uses shallow equality to prevent re-renders when the node data hasn't changed
 */
export const selectNodeById = (nodeId: string) => (state: EditorStore) => {
