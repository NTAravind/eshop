...
import { defaultGlobalLayout } from '@/modules/storefront/defaults/global-layout';
import { defaultHomePage } from '@/modules/storefront/defaults/home-page';
import { defaultCollectionPage } from '@/modules/storefront/defaults/collection-page';
import { defaultPdpTemplate } from '@/modules/storefront/defaults/pdp-template';
import { defaultCheckoutPage } from '@/modules/storefront/defaults/checkout-page';
import { defaultOrdersPage } from '@/modules/storefront/defaults/orders-page';
import { defaultProfilePage } from '@/modules/storefront/defaults/profile-page';
import { defaultLoginPage } from '@/modules/storefront/defaults/login-page';
...
import * as settingsDal from '@/dal/storefront-settings.dal';
...
/**
 * Get published prefabs as a map (key -> tree)
 */
export async function getPublishedPrefabs(storeId: string) {
    const docs = await documentDal.getDocumentsByStore(
        storeId,
        StorefrontDocKind.PREFAB,
        StorefrontDocStatus.PUBLISHED
    );

    return docs.reduce((acc, doc) => {
        acc[doc.key] = doc.tree as unknown as StorefrontNode;
        return acc;
...
// ==================== SETTINGS (DELIVERY, ETC) ====================

/**
 * Get storefront settings
 */
export async function getSettings(storeId: string) {
    const settings = await settingsDal.getStorefrontSettings(storeId);
    if (!settings) return null;

    return {
        deliveryModes: settings.deliveryModes as ('DELIVERY' | 'PICKUP')[] | undefined,
        checkoutFields: settings.checkoutFields as Record<string, any> | undefined,
        profileFields: settings.profileFields as Record<string, any> | undefined,
    };
}
 ...
 */
export async function saveSettings(storeId: string, settings: any) {
    return settingsDal.upsertStorefrontSettings(storeId, settings);
}
...
export async function createDefaultDocuments(storeId: string) {
    const defaultDocs = [
        ...
        { kind: StorefrontDocKind.PAGE, key: 'CART', tree: { id: 'cart-root', type: 'Container', props: {}, styles: {}, children: [] } }, // Basic cart page placeholder
