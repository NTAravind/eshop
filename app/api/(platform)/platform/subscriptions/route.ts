 ...
 * Create a new subscription (initialize with FREE plan)
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const existingSubscription = await subscriptionDal.getSubscriptionByAccountId(account.id);
        if (existingSubscription) {
            return NextResponse.json(
                { error: 'Subscription already exists for this account' },
                { status: 400 }
            );
        }

        // Initialize with FREE plan
        const subscription = await subscriptionService.initializeFreeSubscription(account.id);
